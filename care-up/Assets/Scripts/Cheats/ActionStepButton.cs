...
public class ActionStepButton : MonoBehaviour
{
    ...
    public void setAction(Action a)
    {
        action = a;
        if (main_text != null)
            main_text.text = "[" + action.info.sequentialNumber.ToString() + "] " +  action.SubIndex.ToString() + " " + action.info.shortDescr;
    }
        ...
        if (actionsPanel != null)
        {
            if (lastMode != actionsPanel.mode)
            {
                ...
                else if (lastMode == ActionsPanel.Mode.Type)
                {
                    ...
                    ss += " sp: " + action.info.secondPlaceRequirement;
                    main_text.text = "[" + action.info.sequentialNumber.ToString() + "] " + ss;
                }
                else if (lastMode == ActionsPanel.Mode.Comment)
                {
                    main_text.text = "[" + action.info.sequentialNumber.ToString() + "] " + action.info.comment;
                }
                else if (lastMode == ActionsPanel.Mode.CommentUA)
                {
                    main_text.text = "[" + action.info.sequentialNumber.ToString() + "] " + action.info.commentUA;
                }
                else if (lastMode == ActionsPanel.Mode.Icons)
                {
                    main_text.text = "[" + action.info.sequentialNumber.ToString() + "] " + action.Type.ToString();
            ...
            {
                main_text.text = "[" + action.info.sequentialNumber.ToString() + "] " + "l:" + action.info.leftHandRequirement + " r:" + action.info.rightHandRequirement + " p:" + action.info.placeRequirement;
            }
