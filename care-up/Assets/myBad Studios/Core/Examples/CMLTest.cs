...
public class CMLTest : MonoBehaviour {
	...
	void Start()
	{
		...
		//let's split this libary up and give it another section
		database.AddNode ("/Section");
		database.AddNode("Section", "Cat=SelfHelp");

		//and now let's make a new book node...
		database.AddNode("Book", "name=CML for Dummies; price=999.95; author=DrDude");
		database.Last.AddToData("CML is so easy I don't know what to write in this book...");
		database.Last.AddToData("The end");

		//I never ended off the first book and want to add some text to it.
		//Fortunately, I know the node index so I can access it directly. No navigation needed.
		...
		foreach(int i in cats)
		{
			...
			Debug.Log("---This section has " + books.Count + " books");
